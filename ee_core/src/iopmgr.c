...
int New_Reset_Iop(const char *arg, int arglen)
{
    ...
    if (iop_reboot_count >= 2 && config->EnablePadEmuOp) {
        ...
        if (config->PadEmuModules & (1 << 0)) {
            LoadOPLModule(OPL_MODULE_ID_DS3USB, 0, 0, NULL);
        }
        if (config->PadEmuModules & (1 << 1)) {
            if (!btstack_loaded) {
        ...
        if (config->PadEmuModules & (1 << 2)) {
            LoadOPLModule(OPL_MODULE_ID_DS4USB, 0, 0, NULL);
        }
        if (config->PadEmuModules & (1 << 3)) {
            if (!btstack_loaded) {
                LoadOPLModule(OPL_MODULE_ID_BTSTACK, 0, 0, NULL);
                btstack_loaded = 1;
            }
